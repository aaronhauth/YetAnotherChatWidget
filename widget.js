...
// track changes in the page's visibility state. while the page is visible, we allow the use of animations.
// This prevents a weird issue where if your chat widget is hidden and reshown and all the messages animate in at once.
var visible = true;
...
window.addEventListener('onEventReceived', function (obj) {
    ...
    if (fieldData.duration > 0) {
        inFlightMessages[messageData.msgId] = {
            timeoutId: setTimeout(removeDomMessage(domMessage), fieldData.duration * 1000),
...
function removeDomMessage(domMessage) {
    return function () {
        ...
        // add outro animation, if config exists for it.
        // also, prevent the animations from applying if the widget isn't visible
        if (visible && fieldData.oldMessageAnimation && fieldData.oldMessageAnimation != "none") {
...
// for a given user, remove all their messages from chat
...
// for a given message, delete it from the chat
...
function generateClassName(animationName, animationDirection) {
    let className = "";
    if (visible && animationName && animationName != "none") { // make sure to skip if animations arent wanted
...
// a utility method to quickly get the dom id (as opposed to the messageId)
...
// a bunch of test messages! one of these messages will be sent at random when you click the "test message" button
const testMessages = [
    {
        "nick": "aaroniush",
        "userId": "43658519",
